...
def location_name(
    ...
    """
    error_string = (
        "Argument latlon must be a tuple: (latitude [degrees N],longitude [degrees E])"
    )
    ...
    if loc_part not in location.raw["address"]:
        ...
        exception_string = f"""For location ({latlon[0]},{latlon[1]}),
        descriptor 'loc_part' argument must be one of {*loc_descriptors,}"""
...
def get_state_fips(
    ...
    """gets the fips value for a state"""
...
def city_lat_lon(
    ...
    try:
        geolocator = Nominatim(user_agent="myapplication", timeout=timeout)
